    ...
    dom.onReady(() => {
      ...
      dom.addEvent(fileInput, "change", (e) => {
        ...
        loadJSONFile(
          file,
          (data) =>
            data.grammar || data.numbers || data.cases || data.vocabulary,
          (data) => {
            exercises = data;
            storage.setJSON("exercises", exercises);
            safeModal(
              "Exercises loaded successfully! Now choose a category to begin.",
              { autoClose: 2500 }
            );
          ...
          (msg) => safeModal("Error loading JSON: " + msg)
        );
      });
      ...
      dom.addEvent(dom.qs("#reset-btn"), "click", () => {
        ...
        if (container)
          container.textContent = "Load the exercises file to start";
        if (fileInput) fileInput.value = "";
        const select = dom.qs("#exercise-type");
        if (select) select.value = "selectType";
        ...
        safeModal("Exercises have been reset!", { autoClose: 2000 });
        updateCounts();
      });
      ...
      dom.addEvent(exerciseTypeSelect, "change", (e) => {
        ...
        if (type === "selectType") {
          if (exampleEl) exampleEl.textContent = "";
          safeModal("Please select a valid exercise type.");
          return;
        }
      ...
      dom.addEvent(dom.qs("#add-exercise-btn"), "click", () => {
        ...
        if (type === "selectType") {
          safeModal(
            "Please select a valid exercise type before adding an exercise."
          );
          return;
        }
        if (!polish || !answer) {
        ...
        const exists = exercises[type].some((item) => {
          if (type === "vocabulary" || type === "flashcards")
            return (
              item.polish.toLowerCase() === normPolish &&
              item.english.toLowerCase() === normAnswer
            );
          return (
            item.question.toLowerCase() === normPolish &&
            item.answer.toLowerCase() === normAnswer
          );
        });
        if (exists) {
          safeModal("This exercise already exists in " + type + ".");
          return;
        }
        if (type === "vocabulary" || type === "flashcards")
          exercises[type].push({ polish, english: answer });
        else exercises[type].push({ question: polish, answer });
        storage.setJSON("exercises", exercises);
        dom.qs("#exercise-polish").value = "";
        dom.qs("#exercise-answer").value = "";
        safeModal(`New exercise added to "${type}"!`, {
          autoClose: 2000,
        });
        const recent = dom.qs("#recent-added");
        if (recent) {
          const label =
            type === "vocabulary" || type === "flashcards"
              ? `${polish} = ${answer}`
              : `${polish} → ${answer}`;
      ...
      dom.addEvent(dom.qs("#download-btn"), "click", () => {
        if (!exercises || Object.keys(exercises).length === 0) {
          safeModal(
            "No exercises to download. Please load or add exercises first.",
            { autoClose: 3500 }
          );
          return;
        }
        ...
        link.href = url;
        link.download = "polish_exercises.json";
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
        URL.revokeObjectURL(url);

        safeModal("Exercises JSON downloaded successfully!", {
          autoClose: 2000,
        });
      });
      ...
      const infoButtons = [
        dom.qs("#info-btn"),
        dom.qs("#info-guide-btn"),
      ].filter(Boolean);
      const jsonSection = dom.qs("#json-format");
      infoButtons.forEach((btn) =>
        dom.addEvent(btn, "click", () => {
          if (jsonSection)
...
function showQuestion() {
  ...
  if (!data) {
    ...
    window.PolishApp &&
      window.PolishApp.effects &&
      window.PolishApp.effects.fadeIn(container);
...
function updateCounts() {
  ...
  Object.entries(selectors).forEach(([key, sel]) => {
    ...
    const base =
      ...
      btn.textContent.replace(/\s*\(\d+\)$/, "");
    if (!btn.getAttribute("data-base-label"))
      btn.setAttribute("data-base-label", base);
    const arr =
      exercises && Array.isArray(exercises[key]) ? exercises[key] : [];
